...
from ..schemas import AnalyzeRequest, AnalyzePageRequest, AnalyzeResponse, RedFlag
from ..models import Scan
from ..database import get_db
from ..services.analyzer import analyze_message
from ..services.url_checker import CompositeURLChecker
...
async def analyze_page_endpoint(request: AnalyzePageRequest, db: Session = Depends(get_db)):
    ...
    # Combine text and page title for evaluation
    ...
    # Check links reputation
    all_urls = list(set([request.page_url] + links_to_scan))
    if all_urls:
        rep_results = await url_checker.check_urls(all_urls)
        for rep in rep_results:
            result["red_flags"].append({
